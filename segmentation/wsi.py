import base
from wsi_handler import get_file_handler
from utils import (
    cropping_center,
    get_bounding_box,
    log_debug,
...
def _get_patch_top_left_info(img_shape, input_size, output_size):
    ...
    output_tl_y_list, output_tl_x_list = np.meshgrid(
        output_tl_y_list, output_tl_x_list)
    output_tl = np.stack(
...
def _get_tile_info(img_shape, tile_shape, ambiguous_size=128):
    ...
    tile_grid_top_left, _ = _get_patch_top_left_info(
        img_shape, tile_shape, tile_shape)
    tile_grid_bot_right = []
    ...
    def stack_coord(x): return np.stack(
        [x[0].flatten(), x[1].flatten()], axis=-1)
    ...
    tile_cross_top_left = np.meshgrid(
        tile_grid_y[1:] - 2 *
        ambiguous_size, tile_grid_x[1:] - 2 * ambiguous_size
    )
...
def _get_chunk_patch_info(
    ...
    """
    def round_to_multiple(x, y): return np.floor(x / y) * y
    ...
    chunk_input_shape = (chunk_output_shape +
                         patch_diff_shape).astype(np.int64)
    ...
    chunk_input_br_list[y_sel,
                        0] += chunk_input_tl_list[y_sel, 0] + patch_diff_shape[0]
    chunk_input_br_list[x_sel,
                        1] += chunk_input_tl_list[x_sel, 1] + patch_diff_shape[1]
    chunk_output_tl_list = chunk_input_tl_list + patch_diff_shape // 2
    chunk_output_br_list = chunk_input_br_list - \
        patch_diff_shape // 2  # may off pixels
...
class InferManager(base.InferManager):
    ...
    def __get_raw_prediction(self, chunk_info_list, patch_info_list):
        ...
        def masking(x, a, b): return (a <= x) & (x <= b)
        for idx in range(0, chunk_info_list.shape[0]):
            ...
            chunk_patch_info_list = self.__select_valid_patches(
                chunk_patch_info_list)

            # there no valid patches, so flush 0 and skip
            if chunk_patch_info_list.shape[0] == 0:
                proc_pool.apply_async(
                    _assemble_and_flush, args=(
                        wsi_pred_map_mmap_path, chunk_info, None)
                )
                continue
    ...
    def _parse_args(self, run_args):
        ...
        self.patch_input_shape = [
            self.patch_input_shape, self.patch_input_shape]
        self.patch_output_shape = [
            self.patch_output_shape, self.patch_output_shape]
        return
    ...
    def process_single_file(self, wsi_path, msk_path, output_dir):
        ...
        if msk_path is not None and os.path.isfile(msk_path):
            ...
            def simple_get_mask():
                ...
                wsi_thumb_rgb = self.wsi_handler.get_full_img(
                    read_mag=scaled_wsi_mag)
        ...
        if np.sum(self.wsi_mask) == 0:
            ...
            return
        if self.save_mask:
            cv2.imwrite("%s/mask/%s.png" %
                        (output_dir, wsi_name), self.wsi_mask * 255)
        if self.save_thumb:
        ...
        start = time.perf_counter()
        tile_coord_set = _get_tile_info(
            self.wsi_proc_shape, tile_shape, ambiguous_size)
        ...
        tile_boundary_info = self.__select_valid_patches(
            tile_boundary_info, False)
        ...
        #######################
        def pbar_creator(x, y): return tqdm.tqdm(
    ...
    def process_wsi_list(self, run_args):
        ...
        for wsi_path in wsi_path_list[:]:
            ...
            if self.save_thumb or self.save_mask:
                output_file = "%s/json/%s.json" % (
                    self.output_dir, wsi_base_name)
            else:
